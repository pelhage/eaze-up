 ...
 */
function composeResponse(patientMessage, { desiredStrains, receivesNotifs }, twiml) {
  ...
  switch (patientCommand) {
    ...
    case 'ADD':
      if (isValidStrain(patientStrain) && desiredStrains.indexOf(patientStrain) === -1) {
        ...
        if (!receivesNotifs) { receivesNotifs === true }
    ...
    case 'REMOVE':
      if (isValidStrain(patientStrain) && desiredStrains.indexOf(patientStrain) > -1) {
        ...
        if (receivesNotifs && !desiredStrains.length) { receivesNotifs === false }
