...
/**
 * This is the main Chat Bot API
 */
function composeResponse(patientMessage, patient, twiml) {
  const { desiredStrains } = patient
  ...
  switch (patientCommand) {
    ...
    case 'ADD':
      if (isValidStrain(patientStrain) && desiredStrains.indexOf(patientStrain) === -1) {
        ...
        if (patient.receivesNotifs !== true) { patient.receivesNotifs = true }
    ...
    case 'REMOVE':
      if (isValidStrain(patientStrain) && desiredStrains.indexOf(patientStrain) > -1) {
        ...
        if (patient.receivesNotifs && !desiredStrains.length) { patient.receivesNotifs = false }
